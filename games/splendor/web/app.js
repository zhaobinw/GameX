...
function cardButton(id, reserved = false) {
  ...
  return `<button class="development ${affordable ? 'affordable' : ''}" data-card="${id}" aria-label="${reserved ? '预留：' : ''}${cardLabel(c)}"><img src="${c.image}" alt="${cardLabel(c)}" loading="eager"></button>`;
}
...
function render() {
  ...
  <section class="players" style="--players:${state.players.length}" aria-label="玩家区域">${state.players.map(p => `<article class="player ${p.id === state.currentPlayer ? 'active' : ''}"><div class="player-head"><strong>玩家 ${p.id + 1}${p.id === state.firstPlayer ? ' · 先手' : ''}</strong><span class="points">${p.score} 分</span></div><div class="token-line">筹码 ${chips(p.tokens)}</div><div class="token-line">折扣 ${chips(p.bonuses)}</div><button data-player="${p.id}">已购 ${p.purchased.length} 张 · 贵族 ${p.nobles.length} 位</button><div class="reserve-row">${p.reserved.map(id => id ? cardButton(id, true) : '<div class="hidden-card">预留暗牌</div>').join('')}</div></article>`).join('')}</section>
  ...
  document.querySelectorAll('[data-gem]').forEach(el => el.onclick = () => {
    ...
    selection[c] = ((selection[c] || 0) + 1) % (maximum + 1); render();
  });
...
$('#new-button').onclick = () => {
  ...
  $('#start-new').onclick = async () => {
    try { await request('/api/new', { players: Number($('#player-count').value), firstPlayer: Number($('#first-player').value), revision }); viewer = null; selection = {}; modal.close(); await refresh(); } catch (e) { error(e.message); }
  };
};
