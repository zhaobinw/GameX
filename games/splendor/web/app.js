...
function render() {
  ...
  $('#table').innerHTML = `${match.mode === 'ai' ? `<p class="ai-banner" role="status">你 vs AI · ${esc(match.label || '攻略版')}第 ${match.generation} 代 · ${state.phase === 'ended' ? '对局结束' : state.currentPlayer === 0 ? '轮到你了' : match.error ? 'AI 暂停：'+esc(match.error)+' <button id="retry-ai">重试</button>' : 'AI 正在思考…'}</p>` : ''}${state.phase === 'ended' ? `<section class="winner"><h2>${state.winners.map(seatName).join('、')}${state.winners.length > 1 ? ' 共同获胜' : ' 获胜'}</h2><p>分数优先；同分时已购发展卡较少者胜，再相同则共享胜利。</p><button id="export-replay">下载复盘</button></section>` : ''}
...
function showNoble(id) {
  ...
  const owner = state.players.find(p => p.nobles.includes(id));
  openModal(`<div class="card-detail"><img src="${n.image}" alt="贵族"><div><h2>贵族 · 3 声望</h2>${owner ? `<p class="purchase-status">已由玩家 ${owner.id + 1} 获得 · 3 分已计入总分</p>` : '<p>回合结束时满足条件自动到访；满足多位时选择一位，每回合最多一位。</p>'}<p>需要已购发展卡提供以下折扣，不消耗卡牌或筹码。</p><div class="payment">${chips(n.cost)}</div>${action ? '<button id="choose-noble" class="primary">选择这位贵族</button>' : ''}</div></div>`);
