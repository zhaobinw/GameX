...
if(!isMainThread){
 ...
 parentPort.on('message',({id,cmd})=>{try{
  ...
  parentPort.postMessage({id,result:{...encode(o,actions),seat:state.currentPlayer,players:state.players.length,ended,winners:state.winners,scores:o.players.map(p=>p.score),decisions,legal:actions,record:ended&&cmd.record?replayRecord(state):undefined}});
 ...
 const workers=Array.from({length:count},()=>new Worker(new URL(import.meta.url))),pending=new Map();let seq=0;
