...
const phaseNames={starting:'准备中',collecting:'并行对弈',optimizing:'更新模型',evaluating:'独立评估',checkpoint_saved:'模型已保存',complete:'已完成',stopped:'已停止',failed:'失败'};
...
const displayLabel=s=>s.replace(/old0/g,'初始模型').replace(/old(\d+)/g,'历史对手 $1').replace(/current/g,'当前模型').replace(/greedy/g,'贪心基线');
...
async function refresh(){
 try{
 const runs=await api('/api/runs');const wanted=selected||runs.reduce((best,r)=>(r.update||0)>(best?.update||0)?r:best,null)?.name||runs[0]?.name||'';
 $('#runs').innerHTML=runs.length?runs.map(r=>`<option value="${esc(r.name)}">${esc(r.name)} · ${phaseNames[r.status]||r.status}</option>`).join(''):'<option value="">暂无训练</option>';
 ...
 $('#games').textContent=rows.reduce((n,r)=>n+r.completed,0);$('#speed').textContent=last?last.decisions_per_second.toFixed(0):'—';$('#win').textContent=pct(last?.evaluation.greedy.win_share??s.evaluation?.win_share);
 $('#message').textContent=s.error||`${s.config?.players||2} 人对局 · ${s.config?.workers||4} 个并行环境 · ${s.config?.parameters?.toLocaleString()||'—'} 个模型参数${['complete','stopped'].includes(s.status)?' · 可继续训练或观看下方对局':''}`;
 $('#history').innerHTML=rows.slice().reverse().map(r=>`<tr><td>${r.update}</td><td>${r.completed} / ${r.truncated}</td><td>${pct(r.evaluation.greedy.win_share)}（${r.evaluation.greedy.completed} 局）</td><td>${pct(r.evaluation.initial.win_share)}</td><td>${r.entropy.toFixed(3)}</td><td>${r.parameter_delta.toFixed(4)}</td></tr>`).join('');
 const points=rows.filter(r=>r.evaluation.greedy.win_share!==null).map((r,i)=>`${10+i*480/Math.max(1,rows.length-1)},${75-r.evaluation.greedy.win_share*65}`).join(' ');
 $('#chart').innerHTML=rows.length?`<svg viewBox="0 0 500 90" role="img" aria-label="贪心基线胜局份额趋势"><path d="M10 10V75H490" fill="none" stroke="#638478"/><polyline points="${points}" fill="none" stroke="#e2c78d" stroke-width="2"/>${points.split(' ').map(p=>`<circle cx="${p.split(',')[0]}" cy="${p.split(',')[1]}" r="3" fill="#e2c78d"/>`).join('')}</svg>`:'尚未完成第一代评估';
...
async function start(resume=false){try{const data={workers:Number($('#workers').value),players:Number($('#players').value),updates:Number($('#updates').value),episodes:Number($('#episodes').value)};if(resume)data.resume=selected;const r=await api('/api/start',data);selected=r.run;resetReplay();$('#message').textContent='训练已启动，正在准备模型…';}catch(e){$('#message').textContent=e.message;}}
