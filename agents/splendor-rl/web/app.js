...
async function refresh(){
 ...
 $('#games').textContent=rows.reduce((n,r)=>n+r.completed,0);$('#speed').textContent=last?last.decisions_per_second.toFixed(0):'—';$('#win-caption').textContent=s.config?.evaluate&&s.config?.opponent==='teacher'?'对攻略教师 · 胜局份额':s.config?.evaluate&&s.config?.opponent==='initial'?'对起点模型 · 胜局份额':'对贪心基线 · 胜局份额';$('#win').textContent=pct(last?.evaluation.greedy.win_share??s.evaluation?.win_share);
 $('#message').textContent=s.error||`${s.config?.teacher_policy?'手写教师评估（非神经网络）':s.config?.knowledge?'攻略预训练 + PPO':'基础 PPO'} · ${s.config?.players||2} 人对局 · ${s.config?.workers||4} 个并行环境 · ${s.config?.parameters?.toLocaleString()||'—'} 个模型参数${['complete','stopped'].includes(s.status)?' · 可继续训练或观看下方对局':''}`;
 $('#history').innerHTML=rows.slice().reverse().map(r=>`<tr><td>${r.update}</td><td>${r.completed} / ${r.truncated}</td><td>${pct(r.evaluation.greedy.win_share)}（${r.evaluation.greedy.completed} 局）</td><td>${pct(r.evaluation.initial.win_share)}</td><td>${pct(r.evaluation.teacher?.win_share)}</td><td>${r.entropy.toFixed(3)}</td><td>${r.parameter_delta.toFixed(4)}</td></tr>`).join('');
